...
export const Select = createChooser({
  render(data, keys, selection, {format = d => d, multiple, size, label, width}) {
    if (typeof format !== "function") throw new TypeError("format is not a function");
    const form = html`<form class=__ns__>
      ${maybeLabel(label)}<select style=${{width: length(width)}} multiple=${multiple} size=${size} name=input>
